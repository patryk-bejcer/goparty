...
//	    return response()->json($events);
	...
	public function takePart( Request $request ) {
		...
		if ( ! Auth::check() ) {
			...
			$eventName = Event::findOrFail( $request->event_id )->title;

			$events = EventAttendance::where( 'user_id', Auth::id() )->get();

			return view( 'dashboard.events.attendance' )->with( 'events', $events )
			                                            ->with( 'flash', 'Świetnie! Bierzesz udział w imprezie ' . $eventName );
		}
...
//			var_dump($event);
			...
			return view( 'dashboard.events.attendance' )
				->with( 'events', $events )
				->with( 'flash', 'Zrezygnowałeś z imprezy ' . $eventName );
		}
